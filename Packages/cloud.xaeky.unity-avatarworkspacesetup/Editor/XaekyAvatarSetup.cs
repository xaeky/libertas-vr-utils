...
public class XaekyCustomAvatarSetup : MonoBehaviour {
  ...
  public static void CreateAvatarWorkspace(string usernamePath = "", string avatarName = "", XaekyAvatarBase avatarBase = null, bool showSceneOnCreation = true) {
    ...
    if (avatarBase != null) {
      ...
      if (!AssetDatabase.IsValidFolder(avatarBase.defaultBaseDir)) {
    ...
    // Setup materials, duplicate them from the avatar base's materials folder if it exists
    if (avatarBaseExists) {
      string matFoldersPath = $"{avatarBase.defaultBaseDir}{avatarBase.defaultMaterialsDir}";
      if (AssetDatabase.IsValidFolder(matFoldersPath)) {
        foreach (var matName in avatarBase.defaultMaterialsNames) {
          var matPath = $"{avatarBase.defaultBaseDir}{avatarBase.defaultMaterialsDir}{matName}.mat";
      ...
      // Duplicate mobile materials if the avatar base has a mobile materials folder and mobile material names defined
      if (!string.IsNullOrEmpty(avatarBase.defaultMaterialsMobileDir) && avatarBase.defaultMaterialsMobileNames != null) {
        string mobileMatFoldersPath = $"{avatarBase.defaultBaseDir}{avatarBase.defaultMaterialsMobileDir}";
        if (AssetDatabase.IsValidFolder(mobileMatFoldersPath)) {
          foreach (var matName in avatarBase.defaultMaterialsMobileNames) {
            var matPath = $"{avatarBase.defaultBaseDir}{avatarBase.defaultMaterialsMobileDir}{matName}.mat";
            if (AssetDatabase.LoadAssetAtPath<Material>(matPath) != null) {
              AssetDatabase.CopyAsset(matPath, $"{fullAvatarPath}/Materials/Quest/{avatarName}.{matName}.mat");
              Debug.Log($"Mobile material \"{matName}\" copied from \"{matPath}\" to \"{fullAvatarPath}/Materials/Quest/{avatarName}.{matName}.mat\".");
            } else {
              Debug.LogWarning($"Mobile material \"{matName}\" not found at path \"{matPath}\". Skipping.");
            }
          }
        }
      }
    ...
    var avatarPrefab = AssetDatabase.LoadAssetAtPath<GameObject>(avatarBaseExists && !string.IsNullOrEmpty(avatarBase.defaultPrefabWindowsPath) ? $"{avatarBase.defaultBaseDir}{avatarBase.defaultPrefabWindowsPath}" : null);
    ...
    if (avatarBaseExists && !string.IsNullOrEmpty(avatarBase.defaultPrefabAndroidPath)) {
      ...
      var avatarQuestPrefab = AssetDatabase.LoadAssetAtPath<GameObject>($"{avatarBase.defaultBaseDir}{avatarBase.defaultPrefabAndroidPath}");
  ...
  public static bool AvatarBaseDirExists(XaekyAvatarBase avatarBase) {
    return AssetDatabase.IsValidFolder(avatarBase.defaultBaseDir);
  }
}
